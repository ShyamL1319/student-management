...
import { Attendance, AttendanceDocument, AttendanceType } from '../attendances/schemas/attendance.schema';
...
export class ParentsService {
  ...
  async register(dto: RegisterParentDto): Promise<any> {
    const existingUser = await this.userModel.findOne({ email: dto.email.toLowerCase() }).setOptions({ bypassTenant: true }).exec();
    if (existingUser) {
    ...
    if (!parentRole) {
      throw new NotFoundException('Parent role has not been seeded in the system');
    }
  ...
  async linkChild(parentIdStr: string, dto: LinkChildDto): Promise<any> {
    ...
    if (!student) {
      throw new NotFoundException(`Student with admission number ${dto.admissionNumber} not found`);
    }
    ...
    if (!studentDate || inputDate.getUTCFullYear() !== studentDate.getUTCFullYear() ||
        inputDate.getUTCMonth() !== studentDate.getUTCMonth() ||
        inputDate.getUTCDate() !== studentDate.getUTCDate()) {
      throw new BadRequestException('Verification failed: Child birth date does not match school files');
    }

    // Link student to parent
    const childrenList = parent.children || [];
    if (!childrenList.some((childId: any) => childId.toString() === student._id.toString())) {
    ...
    const childrenSummaries = await Promise.all(
      children.map(async (child) => {
        ...
        const [attendanceRecords, marks, invoices, exams] = await Promise.all([
          ...
          this.examModel.find({ isPublished: true, ...(classId ? { class: classId } : {}) }).lean(),
        ]);

        const presentCount = attendanceRecords.filter(r => r.status === 'PRESENT').length;
        const attendancePct = attendanceRecords.length > 0 ? Math.round((presentCount / attendanceRecords.length) * 100) : 100;

        const unpaidFees = invoices.reduce((sum, inv) => sum + (inv.pendingAmount || 0), 0);
        const outstandingInvoices = invoices.filter(inv => inv.pendingAmount > 0).length;

        const averageMarks = marks.length > 0 
          ? Math.round((marks.reduce((sum, m) => sum + (m.marksObtained / m.maxMarks), 0) / marks.length) * 100)
          : null;
  ...
  async getChildAttendance(parentIdStr: string, studentId: string, options: { page?: number; limit?: number }) {
    ...
    const [data, total] = await Promise.all([
      this.attendanceModel.find({
        attendeeType: AttendanceType.STUDENT,
        $or: [{ attendeeId: new Types.ObjectId(studentId) }, { student: new Types.ObjectId(studentId) }],
      })
      .sort({ date: -1 })
      .skip(skip)
      .limit(limit)
  ...
  async getChildFees(parentIdStr: string, studentId: string) {
    await this.verifyChildAccess(parentIdStr, studentId);
    return this.invoiceModel.find({ studentId: new Types.ObjectId(studentId) }).sort({ invoiceDate: -1 }).lean();
  }
  ...
  async getMessages(parentIdStr: string) {
    ...
    }).populate('senderId', 'firstName lastName roleType').populate('recipientId', 'firstName lastName roleType').sort({ createdAt: -1 }).lean();
  }
  ...
  async sendMessage(parentIdStr: string, dto: SendParentMessageDto) {
    ...
    if (recipient.roleType !== 'TEACHER' && recipient.roleType !== 'STAFF' && recipient.roleType !== 'ADMIN') {
      throw new ForbiddenException('Parents can only message Teachers, Staff, or School Admins');
    }
  ...
  async getProfile(parentIdStr: string) {
    const parent = await this.userModel.findById(parentIdStr).populate('role').lean();
  ...
  async findOne(id: string): Promise<any> {
    const parent = await this.userModel.findById(id)
      .populate('role')
      .populate({
        path: 'children',
        model: 'User',
  ...
  async create(dto: CreateParentDto): Promise<any> {
    ...
    const childrenObjectIds = (dto.children || []).map((id: string) => new Types.ObjectId(id));
  ...
  async update(id: string, dto: UpdateParentDto): Promise<any> {
    ...
    if (dto.children) {
      updateFields.children = dto.children.map((cid: string) => new Types.ObjectId(cid));
    }
    ...
    if (dto.children) {
      const childrenObjectIds = dto.children.map((cid: string) => new Types.ObjectId(cid));
      await this.studentModel.updateMany(
  ...
  async remove(id: string): Promise<any> {
    ...
    return { success: true, message: `Parent with ID ${id} deleted successfully` };
  }

  // Helper validation guard to prevent parents from accessing unauthorized children info
  private async verifyChildAccess(parentIdStr: string, studentId: string): Promise<void> {
    const parent = await this.userModel.findById(parentIdStr).exec() as any;
    if (!parent) throw new NotFoundException('Parent account not found');

    const childrenIds = parent.children || [];
    const isChildAssociated = childrenIds.some((childId: any) => childId.toString() === studentId);
    if (!isChildAssociated) {
      throw new ForbiddenException('Access denied: You are not authorized to view this student\'s records');
    }
  }
