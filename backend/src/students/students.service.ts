...
export class StudentsService {
  ...
  async findAll(query: any) {
    const { page = 1, limit = 10, search, class: classId, section, parent, isActive } = query;
    const q: any = {};
    if (search) q.$or = [
    ...
    const itemsQuery = this.studentModel.find(q).populate('parent').populate('class').populate('section').sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit);
    const [data, total] = await Promise.all([itemsQuery.exec(), this.studentModel.countDocuments(q).exec()]);
    return { data, total, page, totalPages: Math.ceil(total / limit) };
  }

  async findOne(id: string) {
    const item = await this.studentModel.findById(id).populate('parent').populate('class').populate('section').exec();
  ...
  async promote(id: string, payload: { class?: string; section?: string }) {
    ...
    student.history.push({ class: student.class, section: student.section, at: new Date() } as any);
