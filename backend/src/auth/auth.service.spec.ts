...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should throw UnauthorizedException if credentials are invalid', async () => {
      ...
      await expect(service.login({ email: 'wrong@school.com', password: 'password' })).rejects.toThrow(
    ...
    it('should login directly if MFA is disabled', async () => {
      ...
      jwtService.sign.mockReturnValueOnce('mockedAccessToken').mockReturnValueOnce('mockedRefreshToken');
      (bcrypt.hash as jest.Mock).mockResolvedValue('hashedNewRefreshToken');
      usersService.update.mockResolvedValue({} as any);

      const result = await service.login({ email: 'user@school.com', password: 'password' });
  ...
  describe('refreshTokens', () => {
    it('should rotate tokens and update hash on success', async () => {
      ...
      jwtService.verify.mockReturnValue({ sub: 'userId123' } as any);
      (bcrypt.compare as jest.Mock).mockResolvedValue(true);
      jwtService.sign.mockReturnValueOnce('newAccessToken').mockReturnValueOnce('newRefreshToken');
      (bcrypt.hash as jest.Mock).mockResolvedValue('hashedNewRefreshToken');
      usersService.update.mockResolvedValue({} as any);

      const result = await service.refreshTokens('userId123', 'oldRefreshToken');
    ...
    it('should revoke all tokens and throw on token reuse (breach detection)', async () => {
      ...
      await expect(service.refreshTokens('userId123', 'stolenOldRefreshToken')).rejects.toThrow(
        UnauthorizedException,
      );
  ...
  describe('MFA Setup & Verify', () => {
    ...
    it('should throw BadRequestException if MFA setup is verified with invalid code', async () => {
      ...
      await expect(service.verifyAndEnableMfa('userId123', '000000')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  ...
  describe('verifyMfaLogin', () => {
    it('should login successfully with valid TOTP code', async () => {
      jwtService.verify.mockReturnValue({ sub: 'userId123', isMfaPending: true });
      mockUser.mfaSecret = 'mockedMfaSecret';
      usersService.findById.mockResolvedValue(mockUser);
      (otplib.verify as jest.Mock).mockResolvedValue({ valid: true });
    ...
    it('should login successfully with valid backup recovery code', async () => {
      ...
      (bcrypt.compare as jest.Mock).mockImplementation(async (plain, hashed) => hashed === 'hashedBackupCode1');

      jwtService.sign.mockReturnValueOnce('accessToken').mockReturnValueOnce('refreshToken');
      (bcrypt.hash as jest.Mock).mockResolvedValue('hashedNewRefreshToken');
      usersService.update.mockResolvedValue({} as any);

      const result = await service.verifyMfaLogin('mfaToken123', 'backupCodePlain');
    ...
    it('should throw UnauthorizedException if code is invalid', async () => {
      ...
      await expect(service.verifyMfaLogin('mfaToken123', 'invalid')).rejects.toThrow(
        UnauthorizedException,
      );
    });
  });
