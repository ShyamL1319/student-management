  ...
  async generateReceiptPdf(receiptData: {
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // 1. Branding / Header
        doc.fillColor('#1A237E').fontSize(24).text('SCHOOL MANAGEMENT SYSTEM', { align: 'center' });
        doc.fillColor('#424242').fontSize(10).text('Official Payment Receipt', { align: 'center' });
        doc.moveDown(2);

        // Draw a line
        doc.strokeColor('#E0E0E0').lineWidth(1).moveTo(40, doc.y).lineTo(550, doc.y).stroke();
        doc.moveDown();

        // 2. Receipt metadata grid
        const startY = doc.y;
        doc.fontSize(10).font('Helvetica-Bold').text('Receipt Details:', 40, startY);
        doc.font('Helvetica').text(`Receipt No: ${receiptData.receiptNumber}`, 40, startY + 15);
        doc.text(`Date: ${new Date(receiptData.paymentDate).toLocaleDateString()}`, 40, startY + 30);
        doc.text(`Payment Method: ${receiptData.paymentMethod}`, 40, startY + 45);
        doc.text(`Txn ID: ${receiptData.transactionId || 'N/A'}`, 40, startY + 60);

        doc.font('Helvetica-Bold').text('Student Details:', 300, startY);
        doc.font('Helvetica').text(`Name: ${receiptData.studentName}`, 300, startY + 15);
        doc.text(`Roll No: ${receiptData.studentRollNumber || 'N/A'}`, 300, startY + 30);
        ...
        const table = {
          ...
          headers: ['Fee Item Description', 'Base Amount', 'Discount', 'Paid Amount'],
          rows: rows,
        };

        doc.table(table, {
          prepareHeader: () => doc.font('Helvetica-Bold').fontSize(9).fillColor('#1A237E'),
          prepareRow: () => doc.font('Helvetica').fontSize(9).fillColor('#424242'),
        });
        ...
        const endY = doc.y;
        doc.strokeColor('#E0E0E0').lineWidth(1).moveTo(40, endY).lineTo(550, endY).stroke();
        doc.moveDown();

        doc.fontSize(12).font('Helvetica-Bold').fillColor('#1A237E')
           .text(`Total Paid: ${receiptData.currency} ${receiptData.amountReceived.toFixed(2)}`, 40, doc.y + 10, { align: 'right' });

        doc.moveDown(3);
        doc.fontSize(8).font('Helvetica-Oblique').fillColor('#757575')
...
School Administration`;
      ...
      this.logger.error(`Failed to generate and email receipt to ${recipientEmail}:`, error);
      return false;
    }
  }
}
