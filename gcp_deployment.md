...
This document outlines the step-by-step process for deploying the School Management application from a local environment to a production environment on Google Cloud Platform (GCP). It covers Secrets Management, Infrastructure Setup, CI/CD configuration, and specific instructions for Cloud Run (Serverless) and GKE (Google Kubernetes Engine).
...
3. Create a secret named `MONGODB_URI`:
   - **Secret value:** Your MongoDB Atlas connection string (e.g., `mongodb+srv://<user>:<pass>@cluster.mongodb.net/school-management`).
...
3. Configuration:
   - **Name:** `school-management-repo`
   - **Format:** Docker
   - **Location Type:** Region (e.g., `us-central1`)
4. Note the repository path. It will look like:
   `us-central1-docker.pkg.dev/YOUR_PROJECT_ID/school-management-repo`
...
1. **Deploy the Backend:**
   ...
   gcloud run deploy school-management-backend \
     --image us-central1-docker.pkg.dev/YOUR_PROJECT_ID/school-management-repo/backend:latest \
     --region us-central1 \
...
2. **Deploy the Frontend:**
   ...
   ```bash
   gcloud run deploy school-management-frontend \
     --image us-central1-docker.pkg.dev/YOUR_PROJECT_ID/school-management-repo/frontend:latest \
     --region us-central1 \
     --allow-unauthenticated \
     --port 80
   ```
...
1. **Create a GKE Cluster:**
   ```bash
   gcloud container clusters create school-management-cluster \
     --region us-central1 \
     --num-nodes 2
   ```
2. **Connect `kubectl` to GKE:**
   ```bash
   gcloud container clusters get-credentials school-management-cluster --region us-central1
   ```
...
4. **Apply Deployment Manifests:**
   ...
   - Change the `image:` fields to point to your GCP Artifact Registry: `us-central1-docker.pkg.dev/YOUR_PROJECT_ID/school-management-repo/backend:latest`
   ```bash
...
2. **Seeding Initial Data:**
   ...
   - **For GKE:**
     ```bash
     kubectl exec -it <backend-pod-name> -n school-management -- sh -c "node seed.js"
     ```
   - **For Cloud Run:**
